...
b)};a[l]=a[l]||{};a[l][b]=function(m,g){arguments.length&&this._createWidget(m,g)};f=new f;f.options=a.extend(true,{},f.options);a[l][b].prototype=a.extend(true,f,{namespace:l,widgetName:b,widgetEventPrefix:a[l][b].prototype.widgetEventPrefix||b,widgetBaseClass:j},h);a.widget.bridge(b,a[l][b])};a.widget.bridge=function(b,f){a.fn[b]=function(h){var l=typeof h==="string",j=Array.prototype.slice.call(arguments,1),m=this;h=!l&&j.length?a.extend.apply(null,[true,h].concat(j)):h;if(l&&h.charAt(0)==="_")return m;
l?this.each(function(){var g=a.data(this,b);if(!g)throw"cannot call methods on "+b+" prior to initialization; attempted to call method '"+h+"'";if(!a.isFunction(g[h]))throw"no such method '"+h+"' for "+b+" widget instance";var i=g[h].apply(g,j);if(i!==g&&i!==e){m=i;return false}}):this.each(function(){var g=a.data(this,b);g?g.option(h||{})._init():a.data(this,b,new f(h,this))});return m}};a.Widget=function(b,f){arguments.length&&this._createWidget(b,f)};a.Widget.prototype={widgetName:"widget",widgetEventPrefix:"",
...
(function(a,e){function c(j){var m=j.charAt(0).toUpperCase()+j.substr(1);j=(j+" "+f.join(m+" ")+m).split(" ");for(var g in j)if(b[g]!==e)return true}a.media=function(){var j={},m=a("html"),g=a("<div id='jquery-mediatest'>"),i=a("<body>").append(g);return function(n){if(!(n in j)){var o=a("<style type='text/css'>@media "+n+"{#jquery-mediatest{position:absolute;}}</style>");m.prepend(i).prepend(o);j[n]=g.css("position")==="absolute";i.add(o).remove()}return j[n]}}();var d=a("<body>").prependTo("html"),
...
"//"+location.host+location.pathname+"ui-dir/",m=a("<base>",{href:j}).appendTo("head"),g=a("<a href='testurl'></a>").prependTo(d)[0].href;m.remove();return g.indexOf(j)===0}()});d.remove();a.support.boxShadow||a("html").addClass("ui-mobile-nosupport-boxshadow")})(jQuery);
(function(a,e){a.each("touchstart touchmove touchend orientationchange tap taphold swipe swipeleft swiperight scrollstart scrollstop".split(" "),function(h,l){a.fn[l]=function(j){return j?this.bind(l,j):this.trigger(l)};a.attrFn[l]=true});var c=a.support.touch,d=c?"touchstart":"mousedown",b=c?"touchend":"mouseup",f=c?"touchmove":"mousemove";a.event.special.scrollstart={enabled:true,setup:function(){function h(g,i){j=i;var n=g.type;g.type=j?"scrollstart":"scrollstop";a.event.handle.call(l,g);g.type=
n}var l=this,j,m;a(l).bind("touchmove scroll",function(g){if(a.event.special.scrollstart.enabled){j||h(g,true);clearTimeout(m);m=setTimeout(function(){h(g,false)},50)}})}};a.event.special.tap={setup:function(){var h=this,l=a(h);l.bind("mousedown touchstart",function(j){function m(q){if(q.type=="scroll")g=true;else{q=q.type=="touchmove"?q.originalEvent.touches[0]:q;if(Math.abs(u[0]-q.pageX)>10||Math.abs(u[1]-q.pageY)>10)g=true}}if(j.which&&j.which!==1||l.data("prevEvent")&&l.data("prevEvent")!==j.type)return false;
l.data("prevEvent",j.type);setTimeout(function(){l.removeData("prevEvent")},800);var g=false,i=true,n=j.target,o=j.originalEvent,u=j.type=="touchstart"?[o.touches[0].pageX,o.touches[0].pageY]:[j.pageX,j.pageY],x,v;v=setTimeout(function(){if(i&&!g){x=j.type;j.type="taphold";a.event.handle.call(h,j);j.type=x}},750);a(window).one("scroll",m);l.bind("mousemove touchmove",m).one("mouseup touchend",function(q){l.unbind("mousemove touchmove",m);a(window).unbind("scroll",m);clearTimeout(v);i=false;if(!g&&
n==q.target){x=q.type;q.type="tap";a.event.handle.call(h,q);q.type=x}})})}};a.event.special.swipe={setup:function(){var h=a(this);h.bind(d,function(l){function j(n){if(g){var o=n.originalEvent.touches?n.originalEvent.touches[0]:n;i={time:(new Date).getTime(),coords:[o.pageX,o.pageY]};Math.abs(g.coords[0]-i.coords[0])>10&&n.preventDefault()}}var m=l.originalEvent.touches?l.originalEvent.touches[0]:l,g={time:(new Date).getTime(),coords:[m.pageX,m.pageY],origin:a(l.target)},i;h.bind(f,j).one(b,function(){h.unbind(f,
j);if(g&&i)if(i.time-g.time<1E3&&Math.abs(g.coords[0]-i.coords[0])>30&&Math.abs(g.coords[1]-i.coords[1])<75)g.origin.trigger("swipe").trigger(g.coords[0]>i.coords[0]?"swipeleft":"swiperight");g=i=e})})}};(function(h){function l(){var i=m();if(i!==g){g=i;j.trigger("orientationchange")}}var j=h(window),m,g;h.event.special.orientationchange={setup:function(){if(h.support.orientation)return false;g=m();j.bind("resize",l)},teardown:function(){if(h.support.orientation)return false;j.unbind("resize",l)},
add:function(i){var n=i.handler;i.handler=function(o){o.orientation=m();return n.apply(this,arguments)}}};m=function(){var i=document.documentElement;return i&&i.clientWidth/i.clientHeight<1.1?"portrait":"landscape"}})(jQuery);a.each({scrollstop:"scrollstart",taphold:"tap",swipeleft:"swipe",swiperight:"swipe"},function(h,l){a.event.special[h]={setup:function(){a(this).bind(l,a.noop)}}})})(jQuery);
(function(a,e,c){function d(g){g=g||location.href;return"#"+g.replace(/^[^#]*#?(.*)$/,"$1")}var b="hashchange",f=document,h,l=a.event.special,j=f.documentMode,m="on"+b in e&&(j===c||j>7);a.fn[b]=function(g){return g?this.bind(b,g):this.trigger(b)};a.fn[b].delay=50;l[b]=a.extend(l[b],{setup:function(){if(m)return false;a(h.start)},teardown:function(){if(m)return false;a(h.stop)}});h=function(){function g(){var q=d(),w=v(o);if(q!==o){x(o=q,w);a(e).trigger(b)}else if(w!==o)location.href=location.href.replace(/#.*/,
"")+w;n=setTimeout(g,a.fn[b].delay)}var i={},n,o=d(),u=function(q){return q},x=u,v=u;i.start=function(){n||g()};i.stop=function(){n&&clearTimeout(n);n=c};a.browser.msie&&!m&&function(){var q,w;i.start=function(){if(!q){w=(w=a.fn[b].src)&&w+d();q=a('<iframe tabindex="-1" title="empty"/>').hide().one("load",function(){w||x(d());g()}).attr("src",w||"javascript:0").insertAfter("body")[0].contentWindow;f.onpropertychange=function(){try{if(event.propertyName==="title")q.document.title=f.title}catch(t){}}}};
i.stop=u;v=function(){return d(q.location.href)};x=function(t,z){var r=q.document,G=a.fn[b].domain;if(t!==z){r.title=f.title;r.open();G&&r.write('<script>document.domain="'+G+'"<\/script>');r.close();q.location.hash=t}}}();return i}()})(jQuery,this);
...
(function(a){a.fn.fixHeaderFooter=function(){if(!a.support.scrollTop)return a(this);return a(this).each(function(){a(this).data("fullscreen")&&a(this).addClass("ui-page-fullscreen");a(this).find('.ui-header[data-position="fixed"]').addClass("ui-header-fixed ui-fixed-inline fade");a(this).find('.ui-footer[data-position="fixed"]').addClass("ui-footer-fixed ui-fixed-inline fade")})};a.fixedToolbars=function(){function e(g){var i=0;if(g){var n=g.offsetParent,o=document.body;for(i=g.offsetTop;g&&g!=o;){i+=
g.scrollTop||0;if(g==n){i+=n.offsetTop;n=g.offsetParent}g=g.parentNode}}return i}function c(g){var i=a(window).scrollTop(),n=e(g[0]),o=g.css("top")=="auto"?0:parseFloat(g.css("top")),u=window.innerHeight,x=g.outerHeight(),v=g.parents(".ui-page:not(.ui-page-fullscreen)").length;if(g.is(".ui-header-fixed")){o=i-n+o;if(o<n)o=0;return g.css("top",v?o:i)}else{o=i+u-x-(n-o);return g.css("top",v?o:i+u-x)}}if(a.support.scrollTop){var d="inline",b,f=a.support.touch,h=f?"touchstart":"mousedown",l=f?"touchend":
"mouseup",j=null,m=false;a(function(){a(document).bind(h,function(g){if(!a(g.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length){j=d;a.fixedToolbars.hide(true)}}).bind("scrollstart",function(g){if(!a(g.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length){m=true;if(j==null)j=d;a.fixedToolbars.hide(true)}}).bind(l,function(g){if(!a(g.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length)if(!m){a.fixedToolbars.toggle(j);
...
a(g.target).find('[data-role="footer"]').addClass("ui-footer-duplicate");b.appendTo(a.pageContainer).css("top",0);c(b)}});a(".ui-page").live("pageshow",function(g){b&&b.length&&b.appendTo(g.target).css("top",0);a.fixedToolbars.show(true,this)})});return{show:function(g,i){d="overlay";return(i?a(i):a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var n=a(this),o=a(window).scrollTop(),u=
e(n[0]),x=window.innerHeight,v=n.outerHeight();o=n.is(".ui-header-fixed")&&o<=u+v||n.is(".ui-footer-fixed")&&u<=o+x;n.addClass("ui-fixed-overlay").removeClass("ui-fixed-inline");!o&&!g&&n.addClass("in").animationComplete(function(){n.removeClass("in")});c(n)})},hide:function(g){d="inline";return(a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var i=a(this),n=i.css("top");n=n=="auto"?0:
parseFloat(n);i.addClass("ui-fixed-inline").removeClass("ui-fixed-overlay");if(n<0||i.is(".ui-header-fixed")&&n!=0)if(g)i.css("top",0);else i.css("top")!=="auto"&&parseFloat(i.css("top"))!==0&&i.addClass("out reverse").animationComplete(function(){i.removeClass("out reverse");i.css("top",0)})})},hideAfterDelay:function(){setTimeout(function(){a.fixedToolbars.hide()},3E3)},toggle:function(g){if(g)d=g;return d=="overlay"?a.fixedToolbars.hide():a.fixedToolbars.show()}}}}()})(jQuery);
...
(function(a){a.widget("mobile.selectmenu",a.mobile.widget,{options:{theme:null,disabled:false,icon:"arrow-d",iconpos:"right",inline:null,corners:true,shadow:true,iconshadow:true,menuPageTheme:"b",overlayTheme:"a"},_create:function(){var e=this,c=this.options,d=this.element.attr("tabindex","-1").wrap("<div class='ui-select'>"),b=d.attr("id"),f=a("label[for="+b+"]").addClass("ui-select"),h=b+"-button",l=b+"-menu",j=d.closest(".ui-page"),m=a("<a>",{href:"#",role:"button",id:h,"aria-haspopup":"true",
"aria-owns":l}).text(a(d[0].options.item(d[0].selectedIndex)).text()).insertBefore(d).buttonMarkup({theme:c.theme,icon:c.icon,iconpos:c.iconpos,inline:c.inline,corners:c.corners,shadow:c.shadow,iconshadow:c.iconshadow}),g=/ui-btn-up-([a-z])/.exec(m.attr("class"))[1],i=a("<div data-role='dialog' data-theme='"+c.menuPageTheme+"'><div data-role='header'><div class='ui-title'>"+f.text()+"</div></div><div data-role='content'></div></div>").appendTo(a.pageContainer).page(),n=i.find(".ui-content"),o=a("<div>",
{"class":"ui-listbox-screen ui-overlay ui-screen-hidden fade"}).appendTo(j);c=a("<div>",{"class":"ui-listbox ui-listbox-hidden ui-overlay-shadow ui-corner-all pop ui-body-"+c.overlayTheme}).insertAfter(o);var u=a("<ul>",{"class":"ui-listbox-list",id:l,role:"listbox","aria-labelledby":h,"data-theme":g}).appendTo(c);a.extend(e,{select:d,selectID:b,label:f,buttonId:h,menuId:l,thisPage:j,button:m,menuPage:i,menuPageContent:n,screen:o,listbox:c,list:u,menuType:void 0});e.refresh();this.options.disabled&&
this.disable();d.change(function(){e.refresh()}).focus(function(){a(this).blur();m.focus()});m.click(function(){e.open();return false});u.delegate("li","click",function(){var x=u.find("li").index(this),v=d[0].selectedIndex;d[0].selectedIndex=x;x!==v&&d.trigger("change");e.refresh();e.close();return false});o.click(function(){e.close();return false})},_buildList:function(){var e=this;e.list.empty().filter(".ui-listview").listview("destroy");e.select.find("option").each(function(){var c=a("<a>",{role:"option",
...
(function(a){a.widget("mobile.slider",a.mobile.widget,{options:{theme:null,trackTheme:null},_create:function(){function e(t,z){if(z)u=(parseFloat(z)-n)/(o-n)*100;else{var r=t.originalEvent.touches?t.originalEvent.touches[0]:t;if(!w||r.pageX<v.offset().left-8||r.pageX>v.offset().left+v.width()+8)return;u=Math.round((r.pageX-v.offset().left)/v.width()*100)}if(u<0)u=0;if(u>100)u=100;r=Math.round(u/100*(o-n))+n;if(r<n)r=n;if(r>o)r=o;x=Math.round(r/(o-n)*100);q.css("left",u+"%");q.attr({"aria-valuenow":j==
"input"?r:b.find("option").eq(r).attr("value"),"aria-valuetext":j=="input"?r:b.find("option").eq(r).text(),title:r});c(r);r=r;if(j=="input")b.val(r);else b[0].selectedIndex=r;b.trigger("change")}function c(t){if(j!="input")t==0?v.addClass("ui-slider-switch-a").removeClass("ui-slider-switch-b"):v.addClass("ui-slider-switch-b").removeClass("ui-slider-switch-a")}function d(){j=="select"&&q.addClass("ui-slider-handle-snapping").css("left",x+"%").animationComplete(function(){q.removeClass("ui-slider-handle-snapping")})}
var b=this.element,f=b.parents("[class*=ui-bar-],[class*=ui-body-]").eq(0);f=f.length?f.attr("class").match(/ui-(bar|body)-([a-z])/)[2]:"c";var h=this.options.theme?this.options.theme:f,l=this.options.trackTheme?this.options.trackTheme:f,j=b[0].nodeName.toLowerCase();f=j=="select"?"ui-slider-switch":"";var m=b.attr("id"),g=m+"-label";m=a("[for="+m+"]").attr("id",g);var i=j=="input"?parseFloat(b.val()):b[0].selectedIndex,n=j=="input"?parseFloat(b.attr("min")):0,o=j=="input"?parseFloat(b.attr("max")):
b.find("option").length-1,u=(parseFloat(i)-n)/(o-n)*100,x=u,v=a('<div class="ui-slider '+f+" ui-btn-down-"+l+' ui-btn-corner-all" role="application"></div>'),q=a('<a href="#" class="ui-slider-handle"></a>').appendTo(v).buttonMarkup({corners:true,theme:h,shadow:true}).attr({role:"slider","aria-valuemin":n,"aria-valuemax":o,"aria-valuenow":i,"aria-valuetext":i,title:i,"aria-labelledby":g}),w=false;if(j=="select"){v.wrapInner('<div class="ui-slider-inneroffset"></div>');b.find("option");b.find("option").each(function(t){var z=
t==0?"b":"a",r=t==0?"right":"left";t=t==0?" ui-btn-down-"+l:" ui-btn-active";a('<div class="ui-slider-labelbg ui-slider-labelbg-'+z+t+" ui-btn-corner-"+r+'"></div>').prependTo(v);a('<span class="ui-slider-label ui-slider-label-'+z+t+" ui-btn-corner-"+r+'" role="img">'+a(this).text()+"</span>").prependTo(q)})}c(i);m.addClass("ui-slider");b.addClass(j=="input"?"ui-slider-input":"ui-slider-switch").keyup(function(t){e(t,a(this).val())});a(document).bind(a.support.touch?"touchmove":"mousemove",function(t){if(w){e(t);
return false}});v.bind(a.support.touch?"touchstart":"mousedown",function(t){w=true;if(j=="select")i=b[0].selectedIndex;e(t);return false});v.add(document).bind(a.support.touch?"touchend":"mouseup",function(t){if(w){w=false;if(j=="select"){if(i==b[0].selectedIndex){i=i==0?1:0;e(t,i)}d()}return false}});v.insertAfter(b);q.css("left",u+"%").bind("click",function(){return false})}})})(jQuery);
...
a.nodeName(c[0],"ol")},refresh:function(e){this._createSubPages();var c=this.options,d=this.element,b=this,f=d.data("dividertheme")||c.dividerTheme,h=d.children("li"),l=a.support.cssPseudoElement||!a.nodeName(d[0],"ol")?0:1;l&&d.find(".ui-li-dec").remove();h.attr({role:"option",tabindex:"-1"});h.first().attr("tabindex","0");h.each(function(j){var m=a(this),g="ui-li";if(!(!e&&m.hasClass("ui-li"))){var i=m.find("a");if(i.length){m.buttonMarkup({wrapperEls:"div",shadow:false,corners:false,iconpos:"right",
icon:i.length>1?false:m.data("icon")||"arrow-r",theme:c.theme});i.first().addClass("ui-link-inherit");if(i.length>1){g+=" ui-li-has-alt";i=i.last();var n=d.data("splittheme")||i.data("theme")||c.splitTheme;i.attr("title",i.text()).addClass("ui-li-link-alt").empty().buttonMarkup({shadow:false,corners:false,theme:c.theme,icon:false,iconpos:false}).find(".ui-btn-inner").append(a("<span>").buttonMarkup({shadow:true,corners:true,theme:n,iconpos:"notext",icon:d.data("spliticon")||i.data("icon")||c.splitIcon}))}}else if(m.data("role")===
"list-divider"){g+=" ui-li-divider ui-btn ui-bar-"+f;m.attr("role","heading");if(l)l=1}else g+=" ui-li-static ui-btn-up-"+c.theme;if(j===0){if(c.inset){g+=" ui-corner-top";m.add(m.find(".ui-btn-inner")).find(".ui-li-link-alt").addClass("ui-corner-tr").end().find(".ui-li-thumb").addClass("ui-corner-tl")}}else if(j===h.length-1)if(c.inset){g+=" ui-corner-bottom";m.add(m.find(".ui-btn-inner")).find(".ui-li-link-alt").addClass("ui-corner-br").end().find(".ui-li-thumb").addClass("ui-corner-bl")}l&&g.indexOf("ui-li-divider")<
0&&m.find(".ui-link-inherit").first().addClass("ui-li-jsnumbering").prepend("<span class='ui-li-dec'>"+l++ +". </span>");m.addClass(g);e||b._itemApply(d,m)}})},_createSubPages:function(){var e=this.element,c=e.closest(".ui-page"),d=c.attr("id"),b=this.options,f=c.find("[data-role='footer']").data("id");a(e.find("ul, ol").toArray().reverse()).each(function(h){var l=a(this),j=l.parent(),m=j.contents()[0].nodeValue.split("\n")[0];h=d+"&"+a.mobile.subPageUrlKey+"="+a.mobile.idStringEscape(m+" "+h);var g=
l.data("theme")||b.theme,i=l.data("counttheme")||e.data("counttheme")||b.countTheme;l.wrap("<div data-role='page'><div data-role='content'></div></div>").parent().before("<div data-role='header' data-theme='"+b.headerTheme+"'><div class='ui-title'>"+m+"</div></div>").after(f?a("<div>",{"data-role":"footer","data-id":f,"class":"ui-footer-duplicate"}):"").parent().attr({id:h,"data-theme":g,"data-count-theme":i}).appendTo(a.pageContainer).page();j.html("<a href='#"+h+"'>"+m+"</a>")}).listview()}})})(jQuery);
...
(function(a,e,c){function d(k){a.event.special.scrollstart.enabled=false;setTimeout(function(){e.scrollTo(0,k||0)},20);setTimeout(function(){a.event.special.scrollstart.enabled=true},150)}function b(k){k=k.replace(/#/,"").split("/");k.pop();return k.join("/")+(k.length?"/":"")}function f(k){if(k)o.removeClass("ui-loading");else{a.mobile.loadingMessage&&x.appendTo(t).css({top:a(e).scrollTop()+75});o.addClass("ui-loading")}}function h(k){var p=k.find(".ui-title:eq(0)");p.length?p.focus():k.find(G).eq(0).focus()}
function l(k){if(H){k.attr("data-role",H);H=c}}function j(k){var p=["data-role","data-theme","data-fullscreen"],s=k.wrap("<div>").parent();a.each(p,function(y){if(k.attr(p[y])){s.attr(p[y],k.attr(p[y]));k.removeAttr(p[y])}});return s}function m(k){if(z&&(!z.closest(".ui-page-active").length||k))z.removeClass(a.mobile.activeBtnClass);z=null}function g(k,p,s,y){function A(){function E(){f(true);if(C.data("page")._trigger("hide",null,{nextPage:k})!==false&&k.data("page")._trigger("show",null,{prevPage:C})!==
false)a.mobile.activePage=k;h(k);y&&D&&a.mobile.updateHash(D,true);m();K!=null&&K.remove();d(k.data("lastScroll"))}a(e.document.activeElement).blur();var I=n.scrollTop();C.data("lastScroll",I);C.data("page")._trigger("beforehide",{nextPage:k});k.data("page")._trigger("beforeshow",{prevPage:C});if(p&&p!=="none"){t.addClass("ui-mobile-viewport-transitioning");C.addClass(p+" out "+(s?"reverse":""));k.addClass(a.mobile.activePageClass+" "+p+" in "+(s?"reverse":""));k.animationComplete(function(){C.add(k).removeClass(" out in reverse "+
a.mobile.transitions.join(" "));C.removeClass(a.mobile.activePageClass);E();t.removeClass("ui-mobile-viewport-transitioning")})}else{C.removeClass(a.mobile.activePageClass);k.addClass(a.mobile.activePageClass);E()}}function J(E){return E.match("&"+a.mobile.subPageUrlKey)?E.split("&"+a.mobile.subPageUrlKey)[0]:E}var F=a.type(k)==="array",C=F?k[0]:a.mobile.activePage;k=F?k[1]:k;var D=fileUrl=a.type(k)==="string"?k.replace(/^#/,""):null;F=c;var L="get",O=false,K=null;s=s!==c?s:r.length>1&&r[r.length-
2].url===D;p=p!==c?p:a.mobile.defaultTransition;if(a.type(k)==="object"&&k.url){D=k.url;F=k.data;L=k.type;O=true;if(F&&L=="get"){D+="?"+F;F=c}}P();if(s)p=r.pop().transition;else r.push({url:D,transition:p});if(D){k=a("[id='"+D+"']");fileUrl=J(D)}else{var Q=k.attr("id"),R=J(Q);if(Q!=R)fileUrl=R}if(k.length&&!O){fileUrl&&S(fileUrl);l(k);k.page();A()}else{if(k.length)K=k;f();a.ajax({url:fileUrl,type:L,data:F,success:function(E){S(fileUrl);var I=a("<div></div>");I.get(0).innerHTML=E;k=I.find('[data-role="page"]');
if(!a.support.dynamicBaseTag){var U=b(fileUrl||location.hash);k.find("[src],link[href]").each(function(){var T=a(this).is("[href]")?"href":"src",M=a(this).attr(T);M.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(M)||a(this).attr(T,U+M)})}if(k.attr("id"))k=j(k);k.attr("id",fileUrl).appendTo(t);l(k);k.page();A()},error:function(){f(true);m(true);a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",
opacity:0.96,top:a(e).scrollTop()+100}).appendTo(t).delay(800).fadeOut(400,function(){a(this).remove()})}})}}function i(){var k=n.width(),p=[],s=[],y;o.removeClass("min-width-"+B.join("px min-width-")+"px max-width-"+B.join("px max-width-")+"px");a.each(B,function(A){k>=B[A]&&p.push("min-width-"+B[A]+"px");k<=B[A]&&s.push("max-width-"+B[A]+"px")});if(p.length)y=p.join(" ");if(s.length)y+=" "+s.join(" ");o.addClass(y)}a.extend(a.mobile,{subPageUrlKey:"ui-page",nonHistorySelectors:"dialog",activePageClass:"ui-page-active",
activeBtnClass:"ui-btn-active",ajaxLinksEnabled:true,ajaxFormsEnabled:true,transitions:["slide","slideup","slidedown","pop","flip","fade"],defaultTransition:"slide",loadingMessage:"loading",metaViewportContent:"width=device-width, minimum-scale=1, maximum-scale=1",gradeA:function(){return a.support.mediaquery}});a(e.document).trigger("mobileinit");if(a.mobile.gradeA()){var n=a(e),o=a("html"),u=a("head"),x=a.mobile.loadingMessage?a('<div class="ui-loader ui-body-a ui-corner-all"><span class="ui-icon ui-icon-loading spin"></span><h1>'+
a.mobile.loadingMessage+"</h1></div>"):c;a.mobile.metaViewportContent&&a("<meta>",{name:"viewport",content:a.mobile.metaViewportContent}).prependTo(u);var v=b(location.protocol+"//"+location.host+location.pathname),q=a.support.dynamicBaseTag?a("<base>",{href:v}).prependTo(u):c,w,t,z=null,r=[{url:location.hash.replace(/^#/,""),transition:c}],G="[tabindex],a,button:visible,select:visible,input",H=null,N=true,B=[320,480,768,1024];o.addClass("ui-mobile ui-mobile-rendering");a.mobile.urlStack=r;a.mobile.idStringEscape=
function(k){return k.replace(/[^a-zA-Z0-9]/g,"-")};var S=!a.support.dynamicBaseTag?a.noop:function(k){q.attr("href",v+b(k||location.hash))},P=!a.support.dynamicBaseTag?a.noop:function(){q.attr("href",v)};P();a("form").live("submit",function(k){if(a.mobile.ajaxFormsEnabled){var p=a(this).attr("method"),s=a(this).attr("action").replace(location.protocol+"//"+location.host,"");if(!/^(:?\w+:)/.test(s)){if(s.indexOf("/")&&s.indexOf("#")!==0)s=b(location.hash)+s;g({url:s,type:p,data:a(this).serialize()},
c,c,true);k.preventDefault()}}});a("a").live("click",function(k){var p=a(this),s=p.attr("href").replace(location.protocol+"//"+location.host,""),y=p.is("[target]"),A=y||/^(:?\w+:)/.test(s)||p.is("[rel=external]");y=p.is("[target]");if(s==="#")return false;z=p.closest(".ui-btn").addClass(a.mobile.activeBtnClass);if(A||!a.mobile.ajaxLinksEnabled){m(true);if(y)e.open(s);else location.href=s}else{y=p.data("transition");A=p.data("back");var J=!p.is("[data-rel="+a.mobile.nonHistorySelectors+"]");H=p.attr("data-rel");
if(s.indexOf("/")&&s.indexOf("#")!==0)s=b(location.hash)+s;s.replace(/^#/,"");g(s,y,A,J)}k.preventDefault()});a.mobile.updateHash=function(k,p){if(p)N=false;location.hash=k};a(function(){a("body");f();n.bind("hashchange",function(k,p){if(N){if(!a(".ui-page-active").is("[data-role="+a.mobile.nonHistorySelectors+"]")){var s=location.hash,y=p?false:c;if(s)g(s,y);else if(a.mobile.activePage.length&&w[0]!==a.mobile.activePage[0]&&!p)g(w,y,true);else{w.trigger("pagebeforeshow",{prevPage:a("")});w.addClass(a.mobile.activePageClass);
f(true);w.trigger("pageshow",{prevPage:a("")})!==false&&h(w)}}}else N=true})});n.bind("orientationchange.htmlclass",function(k){o.removeClass("portrait landscape").addClass(k.orientation)});n.bind("orientationchange resize",i);i();a.mobile.addResolutionBreakpoints=function(k){if(a.type(k)==="array")B=B.concat(k);else B.push(k);i()};a.fn.animationComplete=function(k){if(a.support.cssTransitions)return a(this).one("webkitAnimationEnd",k);else k()};a.extend(a.mobile,{pageLoading:f,changePage:g,silentScroll:d});
a(function(){var k=a("[data-role='page']");w=a.mobile.activePage=k.first();t=w.parent().addClass("ui-mobile-viewport");a.extend({pageContainer:t});k.page();n.trigger("hashchange",[true]);n.trigger("orientationchange.htmlclass");o.removeClass("ui-mobile-rendering")});n.load(d)}})(jQuery,this);
